	...
	internal abstract class EscapeSequenceTerminal : AbstractTerminal {
		...
		public override void ProcessChar(char ch) {
			...
			if(_processCharResult != ProcessCharResult.Escaping) {
				...
				bool end_flag = false; //escape sequence
				if(_escapeSequence.Length==1) { //ESC+
					end_flag = ('0'<=ch && ch<='9') || ('a'<=ch && ch<='z') || ('A'<=ch && ch<='Z') || ch=='>' || ch=='=' || ch=='|' || ch=='}' || ch=='~';
				}
				else if(_escapeSequence[0]==']') { //OSC (String Terminator)
				...
				if(end_flag) {
					...
					try {
						char code = seq[0];
						_processCharResult = ProcessCharResult.Unsupported; //ProcessEscapeSequence
